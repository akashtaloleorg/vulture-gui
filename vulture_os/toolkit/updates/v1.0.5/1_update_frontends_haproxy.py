...
if __name__ == "__main__":
    ...
    if not node:
        ...
        try:
            frontends = Frontend.objects.filter(mode="log", listening_mode__in=["tcp", "tcp,udp", "relp"])
        except Exception as e:
            print("Failed to retrieve tcp log frontends: {}".format(str(e)))
            sys.exit(0)
        ...
        for frontend in frontends:
            ...
            if node in frontend.get_nodes():
                ...
                if not api_res.get("status"):
                    print("Error while updating HAProxy configuration of frontend '{}': "
