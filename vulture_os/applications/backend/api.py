...
class BackendAPIv1(View):
    ...
    def get(self, request, object_id=None):
        try:
            ...
            if object_id:
                res = Backend.objects.get(pk=object_id).to_dict(fields=fields)

            elif name:
                res = Backend.objects.get(name=name).to_dict(fields=fields)

            elif len(filter) > 0:
                res = [b.to_dict(fields=fields) for b in Backend.objects.filter(filter)]

            else:
                res = [s.to_dict(fields=fields) for s in Backend.objects.all()]
